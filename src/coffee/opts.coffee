...
ezcake::getOpts = (cB)->
  ...
  _.each (@commandModuleArr=[].concat cnf.modules || [], cnf.commands || [], cnf.bundles || []), (v,k)=>
    if (t = _.findWhere ([].concat @uConfig.modules, @uConfig.commands, @uConfig.bundles), name:v.name)?
      ...
      if t.command? and t.command instanceof Array and t.command.length > 1
        ...
        return @error "command element for config #{cnf.name} was #{if typeof t.command == 'undefined' then 'missing' else 'malformed'}" 
      ...
      @error "#{v.name || 'object'} was not defined"
  cB()
#### help()
